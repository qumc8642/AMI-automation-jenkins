...
aws ssm send-command --region "us-east-1" --targets "Key=tag:Name,Values="${DeployName}"" --document-name "AWS-RunShellScript" --parameters commands=["echo Hello world"] --comment "Run unit test sh script" --output-s3-bucket-name "jenkins-log-scratch" --region "us-east-1"'''
      }
    }
    stage('Log Results') {
      steps {
