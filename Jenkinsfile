...
python3 AMICreatePython.py ${DeployName} ${AMIId} ${InstanceType} testInstance isRunning'''
    ...
    stage('Test remote instance') {
      steps {
        sh '''cd /home/jenkins
aws ssm send-command         --targets "Key=tag:Name,Values=tag3"         --document-name "AWS-RunShellScript"         --parameters commands=["bash /home/jenkins/testscript.sh"]         --comment "Run unit test sh script"     --output-s3-bucket-name "jenkins-log-scratch"      --region "us-east-1"'''
      }
    }
    stage('Log Results') {
      steps {
