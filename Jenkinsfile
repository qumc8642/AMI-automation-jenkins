...
python3 AMICreatePython.py ${DeployName} ${AMIId} ${InstanceType} grabID''' , , returnStdout: true).trim()
      ...
      steps {
        sh '''cd /home/jenkins
aws ssm send-command         --instance-ids "${INSTANCE_ID}"         --document-name "AWS-RunShellScript"         --parameters commands=["bash /home/jenkins/unit-test-script.sh"]         --comment "Run unit test sh script"     --output-s3-bucket-name "jenkins-log-scratch"      --region "us-east-1"'''
      }
    }
    stage('Log Results') {
      steps {
