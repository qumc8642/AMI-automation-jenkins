...
cd home/
cd jenkins
aws ssm send-command         --targets "Key=tag:Name,Values=${DeployName}"         --document-name "AWS-RunShellScript"         --parameters commands=["bash /home/jenkins/testscript.sh"]         --comment "Run unit test sh script"         --output-s3-bucket-name "jenkins-log-scratch"'''
      }
    }
    stage('Log Results') {
      steps {
